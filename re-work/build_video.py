#!/usr/bin/env python3
"""
Build the 1-min Clay Ads LinkedIn video.

Talking head segments use Yash footage from source_assets/.
Product demo segments render as black screen with text overlay.
...
AUDIO_DIR = os.path.join(BASE, "audio_segments")
VIDEO_DIR = os.path.join(BASE, "video_segments")
...
SEGMENTS = [
    ...
    {
        "id": 2, "type": "demo", "dur": 7,
        "line": "Clay Ads turns your table into a live audience that syncs directly to LinkedIn, Meta, and now Google.",
        "caption": "Clay Ads - Live Audience Sync",
        "subcaption": "LinkedIn  |  Meta  |  Google",
        "stat": None,
    },
    {
        "id": 3, "type": "head", "dur": 6,
        "line": "We're seeing sixty percent plus match rates on Meta, where most teams are stuck at twenty to thirty percent.",
        "caption": None, "subcaption": None,
        "stat": "60%+ Meta match rate",
    },
    {
        "id": 4, "type": "demo", "dur": 10,
        "line": "With Enhanced Matching, we automatically find and verify the best emails. No more manual enrichment waterfalls.",
        "caption": "Enhanced Matching",
        "subcaption": "Auto-verify the best emails - no manual waterfalls",
        "stat": None,
    },
    {
        "id": 5, "type": "head", "dur": 7,
    ...
    },
    {
        ...
        "caption": "Always-On Pipeline Sync",
        "subcaption": "Leads In  >  Closed Deals Out  >  Auto Sync",
        "stat": None,
    },
    {
        "id": 7, "type": "head", "dur": 7,
        "line": "Teams like Slack and Anthropic are already using this to cut cost per qualified lead by up to five x.",
        "caption": None, "subcaption": None,
        "stat": "5x lower cost per qualified lead",
    },
    {
        "id": 8, "type": "demo", "dur": 8,
        "line": "If you're spending on ads, your audience should be as dynamic as your pipeline.",
        "caption": "Clay Ads",
        "subcaption": "always-on audiences",
        "stat": None,
    },
]
...
def render_text_frame(path, caption=None, subcaption=None, bg="black"):
    """Render a 1280x720 PNG with centered text."""
...
def render_stat_overlay(path, stat_text):
    """Render a transparent-bg stat bar for overlay on talking head."""
...
def step1_audio():
    ...
    for seg in SEGMENTS:
        ...
        run(["say", "-v", VOICE, "-r", "155", "-o", raw, seg["line"]])
        run(["ffmpeg", "-y", "-i", raw,
...
def step2_video():
    ...
    for seg in SEGMENTS:
        ...
        if seg["type"] == "head":
            ...
            if seg["stat"]:
                stat_png = os.path.join(VIDEO_DIR, f"stat_{i}.png")
                render_stat_overlay(stat_png, seg["stat"])
            ...
            render_text_frame(frame_png, caption=seg["caption"], subcaption=seg["subcaption"])
...
def step3_concat():
    ...
    final = os.path.join(OUTPUT_DIR, "clay_ads_v1.mp4")
    run([
